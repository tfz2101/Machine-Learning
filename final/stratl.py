...
class StrategyLearner(object):
    ...
    def addEvidence(self, symbol = "IBM", \
        ...
        for iteration in range(0,10):
            ...
            for i in range(1,indicator.shape[0]):
                ...
                s = action
                s_prime = self.getStateFromDirects([MACD_S[i],RSI_S[i],Boll_S[i],s])
    ...
    def testPolicy(self, symbol = "IBM", \
        ...
        Boll = indicators.getBollingerValues(Stock_Data,30)
        indicator = pd.concat([MACD,RSI,Boll,Stock_Data], axis = 1, join='inner')
        indicator_raw = indicator.dropna()
        indicator = indicator_raw.reset_index(drop=True,inplace=False)

        MACD_S = self.getIndicatorStates(indicator.iloc[:,0],self.MACD_Bins)
        RSI_S = self.getIndicatorStates(indicator.iloc[:,1],self.RSI_Bins)
        Boll_S = self.getIndicatorStates(indicator.iloc[:,2],self.Boll_Bins)

        trades = pd.DataFrame(indicator.iloc[:,3].values,index = indicator_raw.index.values)
        ...
        for i in range(0,indicator.shape[0]):
            ...
            oldPos = curPos
            #print('oldpos',oldPos)
            direct = [MACD_S[i],RSI_S[i],Boll_S[i],curPos]
            state = self.getStateFromDirects(direct)
            curPos = qTab[state,:].argmax()
            #print('q row',qTab[state,:])
            if curPos != oldPos:
                trade = 500 * (curPos - oldPos)
